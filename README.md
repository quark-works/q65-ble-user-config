...
3. Edit [config/q65-ble.conf](config/q65-ble.conf) to add/enable/disable features. Edit [config/q65-ble.keymap](config/q65-ble.keymap) to change the keymap.
...
You can add the q65-ble zmk config repository as a module by modifying your `config/west.yml` file.
...
After making this change, add a copy of [the configuration file](config/q65-ble.conf) and [the keymap](config/q65-ble.keymap) to the `config` folder that is already in your repo.
...
#### Forget the Bluetooth connection on both the q65-ble and the host, then try re-pairing.
